...
Using $libav$, these are combined into an AVI file and saved in 
\emph{path}. To use this option, as with using $createImageSeq$, libav 
...
extension supported by libav (e.g. .MTS, .MP4 or .AVI; see below).
...
requires the installation of Python 2.7 (not 3.6), libav and ExifTool. 
Note that installing Python 2.7 and ExifTool are required \emph{only} 
if you want to use \emph{trackdem} to create image sequences. Libav, in 
...
Python 2.7 release, if not installed yet, can be downloaded from 
...
Finally, Python 2.7 can be downloaded from 
...
directory (which can be set with \emph{setwd()}), provide the full path to the locations. The function creates a 
temporary Python file (tmp.py) in \emph{path} (which is by default set to the working 
directory). Per video, a directory within \emph{imagepath} is created, containing  
.png images. The 
...
files. Linux and Mac users can leave these arguments empty. Windows 
...
USERNAME is your username, or if libav is located elsewhere, supply the 
